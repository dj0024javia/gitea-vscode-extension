...
import type {
    GiteaUser, GiteaPullRequest, GiteaComment, GiteaReview, GiteaFileDiff,
    GiteaCommit, GiteaCombinedStatus, GiteaWorkflowRun, GiteaWorkflowJob,
    GiteaWorkflow, GiteaIssue, GiteaReviewComment, PagedResult
} from './types';
...
export class GiteaApiClient {
    ...
    async createPullRequest(repoInfo: RepoInfo, params: {
        title: string;
        body: string;
        head: string;
        base: string;
    ...
    async mergePullRequest(repoInfo: RepoInfo, number: number, method: 'merge' | 'rebase' | 'squash' = 'merge', message?: string): Promise<void> {
        const { serverUrl, owner, repo } = repoInfo;
    ...
    async updatePullRequest(repoInfo: RepoInfo, number: number, params: { title?: string; body?: string; base?: string }): Promise<GiteaPullRequest> {
        const { serverUrl, owner, repo } = repoInfo;
        return this.request<GiteaPullRequest>(serverUrl, `/repos/${owner}/${repo}/pulls/${number}`, {
            method: 'PATCH',
            body: JSON.stringify(params)
    ...
    async listAllPRReviewComments(repoInfo: RepoInfo, number: number): Promise<GiteaReviewComment[]> {
        const { serverUrl, owner, repo } = repoInfo;
        return this.request<GiteaReviewComment[]>(serverUrl, `/repos/${owner}/${repo}/pulls/${number}/comments`);
    }
    ...
    async rerunWorkflow(repoInfo: RepoInfo, runId: number): Promise<void> {
        const { serverUrl, owner, repo } = repoInfo;
        await this.request<void>(serverUrl, `/repos/${owner}/${repo}/actions/runs/${runId}/rerun`, {
            method: 'POST'
        });
    }
    ...
    async cancelWorkflowRun(repoInfo: RepoInfo, runId: number): Promise<void> {
        const { serverUrl, owner, repo } = repoInfo;
        await this.request<void>(serverUrl, `/repos/${owner}/${repo}/actions/runs/${runId}/cancel`, {
            method: 'POST'
        });
    }
    ...
    async createIssue(repoInfo: RepoInfo, params: {
        title: string;
        body: string;
        assignees?: string[];
        labels?: number[];
