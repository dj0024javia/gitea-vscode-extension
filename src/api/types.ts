...
export interface GiteaReview {
    ...
    user: GiteaUser;
    body: string;
    state: 'APPROVED' | 'REQUEST_CHANGES' | 'COMMENT' | 'REQUEST_REVIEW' | 'REJECTED' | 'pending';
    submitted_at: string;
    stale: boolean;
    html_url: string;
}
...
export interface GiteaWorkflowRun {
    ...
    status: 'unknown' | 'waiting' | 'running' | 'success' | 'failure' | 'cancelled' | 'skipped' | 'blocked' | 'pending';
    conclusion: string;
    workflow_id: string;
    run_number: number;
    event: string;
...
export interface GiteaIssue {
    ...
    updated_at: string;
    closed_at?: string;
    pull_request?: { merged: boolean; diff_url: string; html_url: string; patch_url: string };
}
