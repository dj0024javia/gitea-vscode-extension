...
export class AuthManager {
    ...
    async signIn(serverUrl: string, token: string): Promise<GiteaAccount> {
        ...
        await this.context.secrets.store(`${SECRET_KEY_PREFIX}${normalized}`, token);
    ...
    async getSession(serverUrl?: string): Promise<GiteaAccount | undefined> {
        const accounts = this.getAccountMap();
        const urls = serverUrl ? [serverUrl.replace(/\/$/, '')] : Object.keys(accounts);
        for (const url of urls) {
            const token = await this.context.secrets.get(`${SECRET_KEY_PREFIX}${url}`);
            if (token && accounts[url]) {
                return { serverUrl: url, token, username: accounts[url].username, label: accounts[url].label };
            }
        }
        return undefined;
    }
    ...
    getAccountMap(): Record<string, { username: string; label: string }> {
        return this.context.globalState.get<Record<string, { username: string; label: string }>>(ACCOUNT_MAP_KEY) ?? {};
    }
