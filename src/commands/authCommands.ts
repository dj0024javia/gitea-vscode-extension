...
async function cmdSignIn(
    ...
    await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Signing in to Gitea...' },
        async () => {
            try {
                const account = await auth.signIn(serverUrl, token);
                await vscode.commands.executeCommand('setContext', 'gitea.authenticated', true);
                ...
                if (!cfg.get<string>('serverUrl')) {
                    await cfg.update('serverUrl', account.serverUrl, vscode.ConfigurationTarget.Global);
                }
                vscode.window.showInformationMessage(`Signed in as ${account.username} @ ${serverUrl}`);
                prProvider.refresh();
                ciProvider.refresh();
                statusBar.refresh();
            } catch (err) {
...
async function cmdSignOut(
    ...
    await vscode.commands.executeCommand('setContext', 'gitea.authenticated', remaining.length > 0);
