...
import { PullRequestProvider, PullRequestItem } from '../views/pullRequestProvider';
...
export function registerPRCommands(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('gitea.refreshPRs', () => prProvider.refresh()),

        vscode.commands.registerCommand('gitea.loadMorePRs', (repoKey: string) => {
            prProvider.loadMore(repoKey);
        }),

        vscode.commands.registerCommand('gitea.openPR', async (arg: PullRequestItem | GiteaPullRequest) => {
            const pr = arg instanceof PullRequestItem ? arg.pr : arg;
            await vscode.env.openExternal(vscode.Uri.parse(pr.html_url));
        }),

        vscode.commands.registerCommand('gitea.viewPRDetail', async (pr: GiteaPullRequest, repoInfo: RepoInfo) => {
            await PRDetailPanel.show(context.extensionUri, api, repoInfo, pr);
        }),

        vscode.commands.registerCommand('gitea.checkoutPR', async (pr: GiteaPullRequest, repoInfo?: RepoInfo) => {
        ...
        vscode.commands.registerCommand('gitea.mergePR', async (arg: PullRequestItem) => {
            if (!(arg instanceof PullRequestItem)) { return; }
            await mergePR(api, arg.pr, arg.repoInfo, prProvider);
        }),

        vscode.commands.registerCommand('gitea.approvePR', async (arg: PullRequestItem) => {
            if (!(arg instanceof PullRequestItem)) { return; }
            await reviewPR(api, arg.pr, arg.repoInfo, 'APPROVED', prProvider);
        }),

        vscode.commands.registerCommand('gitea.requestChangesPR', async (arg: PullRequestItem) => {
            if (!(arg instanceof PullRequestItem)) { return; }
            await reviewPR(api, arg.pr, arg.repoInfo, 'REQUEST_CHANGES', prProvider);
        }),

        vscode.commands.registerCommand('gitea.addComment', async (arg?: PullRequestItem) => {
            if (arg instanceof PullRequestItem) {
                ...
                if (!repoInfo) { return; }
                const numStr = await vscode.window.showInputBox({ prompt: 'PR number', validateInput: v => /^\d+$/.test(v) ? null : 'Enter a number' });
                if (!numStr) { return; }
...
async function checkoutPR(pr: GiteaPullRequest, repoInfo?: RepoInfo): Promise<void> {
    ...
    const repo = repoInfo
        ? allRepos.find((r: { rootUri: vscode.Uri }) => r.rootUri.fsPath === repoInfo.rootPath) ?? allRepos[0]
        : allRepos[0];
    ...
    await vscode.window.withProgress(
        ...
        async () => {
            try {
                ...
                try {
                    await repo.checkout(`origin/${branch}`, { createNewBranch: true, newBranchName: branch });
                    vscode.window.showInformationMessage(`Checked out branch: ${branch}`);
                } catch (err2) {
                    vscode.window.showErrorMessage(`Failed to checkout: ${(err2 as Error).message}`);
                }
            }
        }
    );
...
async function createPR(
    ...
    const head = branches.length > 0
        ? await vscode.window.showQuickPick(branches, { placeHolder: 'Head branch (source)' })
        : await vscode.window.showInputBox({ prompt: 'Head branch (source)', value: repoInfo.currentBranch ?? '' });
    if (!head) { return; }

    const base = branches.length > 0
        ? await vscode.window.showQuickPick(branches.filter(b => b !== head), { placeHolder: 'Base branch (target)' })
        : await vscode.window.showInputBox({ prompt: 'Base branch (target)', value: 'main' });
    if (!base) { return; }
    ...
    const body = await vscode.window.showInputBox({ prompt: 'Description (optional)' }) ?? '';

    await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Creating pull request...' },
        async () => {
            try {
                const pr = await api.createPullRequest(repoInfo, { title, body, head, base });
                const action = await vscode.window.showInformationMessage(`PR #${pr.number} created.`, 'Open in Browser');
                if (action === 'Open in Browser') {
                    await vscode.env.openExternal(vscode.Uri.parse(pr.html_url));
                }
                prProvider.refresh();
            } catch (err) {
                vscode.window.showErrorMessage(`Failed to create PR: ${(err as Error).message}`);
            }
        }
    );
}
...
async function mergePR(
    ...
    const method = await vscode.window.showQuickPick(
        [{ label: 'Merge commit', value: 'merge' }, { label: 'Rebase', value: 'rebase' }, { label: 'Squash', value: 'squash' }],
        { placeHolder: 'Select merge method' }
    );
    if (!method) { return; }
    const confirm = await vscode.window.showWarningMessage(
        `Merge PR #${pr.number} (${method.value})?`, { modal: true }, 'Merge'
    );
    ...
    await vscode.window.withProgress(
        ...
        async () => {
            try {
                await api.mergePullRequest(repoInfo, pr.number, method.value as 'merge' | 'rebase' | 'squash');
                vscode.window.showInformationMessage(`PR #${pr.number} merged.`);
                prProvider.refresh();
            } catch (err) {
                vscode.window.showErrorMessage(`Merge failed: ${(err as Error).message}`);
            }
        }
    );
}
...
async function reviewPR(
    ...
    const body = await vscode.window.showInputBox({ prompt: 'Review comment (optional)' }) ?? '';
    await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: `Submitting review...` },
        async () => {
            try {
