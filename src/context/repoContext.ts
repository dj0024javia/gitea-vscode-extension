...
export class RepoContext {
    ...
    private async detectFromWorkspace(): Promise<RepoInfo | undefined> {
        ...
        for (const folder of folders) {
            ...
            if (!fs.existsSync(gitConfigPath)) { continue; }
            try {
    ...
    private parseGitConfig(content: string): RepoInfo | undefined {
        ...
        if (httpsMatch) {
            return { serverUrl: httpsMatch[1], owner: httpsMatch[2], repo: httpsMatch[3] };
        }

        // git@gitea.example.com:owner/repo.git
        const sshMatch = rawUrl.match(/^git@([^:]+):([^/]+)\/([^/]+?)(?:\.git)?$/);
        if (sshMatch) {
            return { serverUrl: `https://${sshMatch[1]}`, owner: sshMatch[2], repo: sshMatch[3] };
        }
