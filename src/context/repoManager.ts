...
function parseRemoteUrl(url: string, rootPath: string, serverUrlOverride?: string): RepoInfo | undefined {
    url = url.trim();
    ...
    if (!owner || !repo || !detectedServerUrl) { return undefined; }

    // Use the user-configured override if set, otherwise use the detected URL
    const serverUrl = serverUrlOverride ? serverUrlOverride.replace(/\/$/, '') : detectedServerUrl;
    return { serverUrl, owner, repo, rootPath, label: `${owner}/${repo}`, key: `${serverUrl}|${owner}/${repo}` };
}
        ...
        if (gitExt && gitExt.isActive) {
            try {
                ...
                for (const gitRepo of gitApi.repositories) {
                    ...
                    const remote =
                        remotes.find((r: { name: string }) => r.name === 'origin') ?? remotes[0];
                    if (!remote) { continue; }
                    const url = (remote.fetchUrl ?? remote.pushUrl ?? '').trim();
                    if (!url) { continue; }
                    const info = parseRemoteUrl(url, gitRepo.rootUri.fsPath, serverUrlOverride);
