...
export class CIDetailPanel {
    ...
    private async handleMessage(msg: { command: string; jobId?: number }): Promise<void> {
        switch (msg.command) {
            case 'refresh':
            ...
            case 'rerun':
                try {
                    ...
                    vscode.window.showErrorMessage(`Rerun failed: ${(err as Error).message}`);
                }
                break;
            case 'cancel':
                try {
    ...
    private renderHtml(run: GiteaWorkflowRun, jobs: GiteaWorkflowJob[]): string {
        const statusIcons: Record<string, string> = {
            success: '✅', failure: '❌', running: '⏳', waiting: '⏸', pending: '⏸',
            cancelled: '🚫', skipped: '⏭', unknown: '❓', blocked: '🔒'
        };
