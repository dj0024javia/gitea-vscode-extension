...
export function iconForStatus(status: string): vscode.ThemeIcon {
    switch (status) {
        ...
        case 'waiting':
        case 'pending': return new vscode.ThemeIcon('watch', new vscode.ThemeColor('charts.yellow'));
        case 'cancelled': return new vscode.ThemeIcon('circle-slash', new vscode.ThemeColor('disabledForeground'));
        case 'skipped': return new vscode.ThemeIcon('debug-step-over', new vscode.ThemeColor('disabledForeground'));
...
export class CIRunItem extends vscode.TreeItem {
    constructor(
        ...
        super(run.display_title || run.name || `Run #${run.run_number}`, vscode.TreeItemCollapsibleState.Collapsed);
...
export class CILoadMoreItem extends vscode.TreeItem {
    constructor(public readonly repoKey: string) {
        ...
        this.command = { command: 'gitea.loadMoreCI', title: 'Load more runs', arguments: [repoKey] };
    }
}
...
export class CIRunsProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
        ...
        if (element instanceof RepoGroupItem) {
            ...
            if (!session) {
                const signIn = new vscode.TreeItem('Sign in to load CI runs', vscode.TreeItemCollapsibleState.None);
    ...
    private async getRepoChildren(repoInfo: RepoInfo): Promise<vscode.TreeItem[]> {
        ...
        if (state.runs.length === 0) {
            const empty = new vscode.TreeItem('No CI runs found', vscode.TreeItemCollapsibleState.None);
            empty.iconPath = new vscode.ThemeIcon('info');
            return [empty];
        }
        const items: vscode.TreeItem[] = state.runs.map(r => new CIRunItem(r, repoInfo));
        if (state.hasMore) { items.push(new CILoadMoreItem(repoInfo.key)); }
        return items;
    }

    private async fetchForRepo(repoInfo: RepoInfo, state: RepoCIState): Promise<void> {
        ...
        try {
            ...
            const result = await this.api.listWorkflowRuns(repoInfo, undefined, state.page, limit);
            state.runs = state.page === 1 ? result.items : [...state.runs, ...result.items];
            state.hasMore = result.hasMore;
        } catch (err) {
            vscode.window.showErrorMessage(`[${repoInfo.label}] Failed to load CI runs: ${(err as Error).message}`);
            state.runs = [];
            state.hasMore = false;
        } finally {
            state.loading = false;
    ...
    private async getJobsForRun(runId: number, repoInfo: RepoInfo): Promise<vscode.TreeItem[]> {
        if (this.jobCache.has(runId)) {
            return (this.jobCache.get(runId) ?? []).map(j => new CIJobItem(j, runId, repoInfo));
        }
        try {
            ...
            vscode.window.showErrorMessage(`Failed to load jobs: ${(err as Error).message}`);
            return [];
        }
    }
}
